...
describe 'Dono de pousada visualiza as reservas' do
  ...
  it 'com estadias ativas' do
    ...
    first_reservation = Reservation.create!(room: room, customer: customer, checkin_date: 1.day.from_now,
                        checkout_date:1.week.from_now, guests_number: 2, status: :active)

    second_reservation = Reservation.create!(room: room, customer: customer, checkin_date: 2.weeks.from_now,
                        checkout_date:1.month.from_now, guests_number: 2)

    travel_to 1.days.from_now
    ActiveReservation.create!(reservation: first_reservation, checkin_date: 1.day.from_now)
    ...
    within room_cards[0] do
      expect(page).to have_content "Data de Check-in: #{I18n.localize(1.day.from_now.to_date)}"
      expect(page).to have_content "Data de Check-out: Pendente"
    end
    travel_back
  end
end